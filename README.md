...
1. **A browser dashboard** (static PWA — no build step, no framework, no backend server) that scans 67 crypto coins on OKX in real time, computes technical signals, shows news, and lets the user ask Claude AI for trade recommendations that can be executed on OKX with one click.
...
2. Each coin gets a score from RSI(1H), MACD, Bollinger %B, volume ratio, and RSI(4H), all read from **finished** candles (the one still forming is ignored). Score ≥ 4.5 → STRONG BUY (≥ 1 in test mode). Scores land on half-points, so 4.5 is one whole step below 5.0, not a fractional nudge.
