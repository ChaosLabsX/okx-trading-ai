...
Quantity math: the worker computes `sz_coin` from **actual fills** (the limit order's filled size plus `remaining / price` for any market-bought remainder; `entry_price` is the spend-weighted average), falling back to `amount_usdt / price` when nothing is known. The browser's manual path still estimates `amount_usdt / price`. Each half is then `sz_coin × 0.5 × 0.9985` — the haircut covers the buy fee having been taken in coin units, so the sells never exceed the actual holdings.
