...
- **Worker hosting:** a **Windows VPS** — `C:\OKXAI`, Task Scheduler task
  ...
  **There is no `.github/workflows/` directory and no CI.** The bot's workflow
  was deleted once the VPS became permanent. Older text in these docs that
  describes the worker running on Actions is historical — the VPS is
  authoritative.

  GitHub Actions *is* still used for exactly one thing: the auto-generated
  `pages-build-deployment` run that publishes the dashboard on every push. It is
  generated by GitHub from the Pages settings, not by any file in this repo.
  **Recent green runs in the Actions tab are Pages deployments and say nothing
  about the bot.**
...
## GitHub Actions workflow (`signal-checker.yml`) — DELETED

> **This workflow no longer exists.** It was removed once the VPS became the
> permanent home for the worker ([`../infra/VPS-SETUP.md`](../infra/VPS-SETUP.md)).
> The description below is kept as a record of how the retired path worked.
>
> To recover the file if it is ever wanted again:
> ```
> git show 151be53:.github/workflows/signal-checker.yml > .github/workflows/signal-checker.yml
> ```
> Note that every secret it referenced is documented in
> [`../infra/.env.example`](../infra/.env.example), so nothing operational was
> lost by deleting it.
